#' @title EBLUPs under Multivariate Fay Herriot Model for non-sampled area
#' @description This function produces EBLUPs and MSE of Multivariate SAE with Difference Benchmarking for non-sampled area
...
#' @export msaefhns
#'
msaefhns <- function (formula, vardir,cluster,nonsample, samevar = FALSE, MAXITER = 100, PRECISION = 1e-04,
                      data) {
  ...
  if (!(TRUE %in% data[,"nonsample"])){
    stop("this msaefhns() function is used for at least 1 observation has no sample, check your 'nonsample' variable or use msaefh() instead ")

  }
  if (length(formula)<=1){
    stop("this msaefhns() function is used for at least 2 response variables, numbers of your response variables is ",length(formula),". use saefhns() function instead")
  }
  r <- length(formula)
