#' @title EBLUPs under Univariate Fay Herriot Model with Difference Benchmarking for non-sampled area
#' @description This function produces EBLUPs, MSE, and aggregation of Univariate SAE with Difference Benchmarking for non-sampled area
...
#' @export saedbns
#'
saedbns <- function (formula, vardir, weight,cluster,nonsample, samevar = FALSE, MAXITER = 100, PRECISION = 1e-04,
                     data) {
  ...
  if (!(TRUE %in% data[,"nonsample"])){
    stop("this saedbns() function is used for at least 1 observation has no sample, check your 'nonsample' variable or use saedb() instead ")
  ...
  result$difference_benchmarking$Estimation_sampled = SAE_DB
  result$difference_benchmarking$Estimation_all = SAE_DB_all
